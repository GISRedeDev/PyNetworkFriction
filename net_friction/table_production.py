...
from .areas_of_control_matrix import (
    calculate_src_dst_areas_of_control
)
...
def process_data(
    ...
    for row in df_matrix.itertuples():
        incidents_in_routes_list.append(get_incidents_in_route(row, pois_df, acled.copy()))
    incidents_in_routes_df = pd.concat(incidents_in_routes_list)
    distances_df = df_matrix[["from_pcode", "to_pcode", "straight_line_distance", "shortest_path_lengths"]]
    ...
    centroids_df.to_crs(f"EPSG:4326", inplace=True)
    centroids_df["geometry"] = get_weighted_centroid(boundaries, raster)
